    ...
    <>
      ...
      <div className="registration-container">
        <form
          ...
          className={
            deviceSize < 720 && !regStates.regWindow ? 'none' : 'registration-container__form'
          }>
          <h1 className="registration-container__form__note">Регистрация</h1>
          <FormControlLogin
            onFocus={handleInputFocus}
            value={regStates.login}
            name="login"
            onChange={handleInputChange}
          />
          <FormControlPswd
            onFocus={handleInputFocus}
            value={regStates.password}
            name="password"
            onChange={handleInputChange}
          ...
          <FormControlPswd
            onFocus={handleInputFocus}
            value={regStates.second_password}
            name="second_password"
            onChange={handleInputChange}
            placeholder="Повторите пароль"
          />
          {load ? <Loader /> : <FormControlSubmit>Зарегистрироваться</FormControlSubmit>}
          {regStates.regError && (
            <p className="registration-container__form__error-label">{regStates.regError}</p>
          )}
          <p className="registration-container__form__link">
            {'Есть аккаунт? Войдите в него! '}
            <NavLink to="/login" className="registration-container__form__link-active">
              Перейти к входу
            </NavLink>
          </p>
          <NavLink
            className="registration-container__form__link registration-container__form__link-active"
            to="/home">
            На главную страницу
          </NavLink>
        </form>
        <div
          className={
            deviceSize < 720 && regStates.regWindow ? 'none' : 'registration-container__avatar-box'
          }>
          <h1 className="registration-container__form__note">Выберите аватар</h1>
          <FormControlImage onChange={handleChangeAvatar}>
            {regStates.avatar?.name || 'Файл не выбран'}
          </FormControlImage>
          {regStates.avatar && (
            <AvatarEditor
              ref={avatarEditorRef}
          ...
          {regStates.avatarError && (
            <p className="registration-container__avatar-box__error-label">
              {regStates.avatarError}
            </p>
          )}
